...
### 4.17 Internal notes and client updates are different fields

A work log carries `internal_notes` (what the team and the reviewer read) and
`client_update` (one optional line, the only thing a spreadsheet can receive).

This was not a refinement, it was a defect fix. The Phase 2 schema split the two
columns precisely because the single `notes` field "was being written to client
sheets" — but no code was ever moved onto the new columns, so `record-work.ts`
kept putting the internal note into the sync payload and the worker kept writing
it into the client's spreadsheet. Migration 0008 moves every existing note into
`internal_notes` and promotes **nothing** to `client_update`: a client-visible
note is now something a person writes on purpose.

The guarantee is structural rather than a filter. `internal_notes` is not
referenced anywhere in `sync-worker.ts`, `sheets.ts` or `sheet-actions.ts`, and
`SHEET_FIELDS` offers no way to map it, so there is no configuration that could
route it to Google. An entry with no client line queues no job at all.

### 4.18 Blocker routing collapsed from thirteen branches to three owners

The original matrix resolved a four-deep role cascade per category and broke
cross-team lead ties by checking which of a reporter's leads was also on the
project. That is right for a company where nobody knows who owns what. For ten
people whose partners speak daily it encoded a hierarchy that does not exist.

Categories now map to one of three owners — sales owner, PM, delivery lead —
with the project role beating the project default, exactly one person copied,
and `ownerSide` kept because client-owned blockers stopping the developer's
clock is genuinely load-bearing. Severity still drives the SLA: that is a
four-entry lookup, and it was never where the complexity was.

Teams are consequently inert. The tables and `/admin/teams` remain, and the page
says plainly that editing them changes nothing.
...
credentials in `.env.local`. Currently attached to **NW-001** only (`sheet_connections` has one row —
`sheet_mappings` was dropped in migration 0008).
...
Remaining caveat: only **append mode** has run against Google. `update` mode has
never run through the worker on a real client template. It no longer guesses a
row from `tasks.sheet_row_ref`; the worker records the row Google reports on
append into `sheet_row_links` and updates that, so a wrong row number is no
longer possible — but that path is still unproven against a live sheet.

Note also that a work log now only queues a sheet write **if the person wrote a
client update line**. An entry with only an internal note reaches no spreadsheet
at all, which is deliberate — see §4.17.

### 9.2 ~~`pnpm db:reset` silently breaks the app~~ — FIXED

`scripts/bootstrap.sh` is now idempotent: it reuses the password already in
`DATABASE_URL` (or `APP_DB_PASSWORD` if set) and only generates one on a genuinely
fresh environment. Re-running it, or resetting the database, leaves `.env.local`
working. Verified by re-running it and reconnecting as `tavren_app`.

### 9.3 ~~No automated tests~~ — PARTLY FIXED

108 tests across 7 files, covering the pure logic: blocker routing (all
categories, fallbacks, watcher rules, SLA), business-time (the shift mapping,
weekend crossing, the negative-input path), RBAC capabilities and project-role
grants, timer derivation, the Sheets A1/column helpers, digest rendering and
webhook chunking. Run with `pnpm verify` (typecheck + lint + tests).

**Still untested:** the RLS backstop and the 404-not-403 access paths. Both need
a database fixture, which the current unit-only setup deliberately has no way to
provide. That is the next test investment.

### 9.4 Nothing runs on a schedule

Unchanged, and still the single thing standing between this and being useful.
`/api/cron/sync`, `/api/cron/sweeps` and `/api/cron/digest` all exist and are
authenticated, but no scheduler calls them. Escalation, stale detection, overrun
detection, health recomputation, sheet sync and the digest happen only when
someone curls them. **Needs a hosting decision.**

### 9.5 ~~Notifications are in-app only~~ — PARTLY FIXED

`POST /api/cron/digest` builds a plain-text status roll-up and posts it to every
webhook in `DIGEST_WEBHOOK_URLS`. Discord and Slack both work; the body shape is
chosen per host. `?dry=1` renders without sending.

**Still open:** per-event delivery (a blocker the moment it is raised) still only
lands in the in-app inbox — only the daily digest goes out. And WhatsApp has no
webhook without the Business API, so for anyone who lives there the digest has
to be pasted. Given the team is split across Discord, WhatsApp and Slack, input
was deliberately converged on one surface (`/log`, installable to a phone) rather
than building a bot per channel.
