...
**The drain lock leaks through the pool.** `runSyncWorker` takes a
*session-level* advisory lock (`sync-worker.ts:490`) and releases it with
`pg_advisory_unlock` in a `finally`. Both go through `db.execute`, which takes
whatever pooled connection is free — so the unlock can be handed a different
backend than the lock was, release nothing, and orphan it. The queue then stops
draining, silently. `scheduler.ts` documents the same hazard and avoids it with
`pg_try_advisory_xact_lock`.

Two corrections to the obvious reading of that, both established by the tests on
the `sales-ops` branch:

- **It is not confined to Sheets, and Google credentials are irrelevant.**
  `reclaimStuckJobs` and `claimJobs` both run before anything reaches the Sheets
  API, and the lock is taken before either. It wedges on a queue that never
  talks to Google at all.
- **It is intermittent, not reliable.** On a quiet run the lock and unlock
  usually land on the same connection and nothing looks wrong. Concurrency
  separates them — and every logged work entry schedules a drain, so
  concurrency is the normal case, not the edge case.

A transaction pooler makes it worse rather than causing it. **The fix is on the
`sales-ops` branch** — the lock and its release are pinned to one reserved
connection. Confirm that has merged before relying on the sheet sync, and delete
this section when it has.
