...
const cells = await p.locator("section a[href*='#entries'], section a[href*='/audit']").allTextContents();
console.log("reconciliation cells:");
cells.forEach(t=>console.log("  ", t.replace(/\s+/g," ").trim()));
console.log("date basis stated:", ((await p.locator("body").textContent())??"").includes("entry date"));
await p.screenshot({path:`${OUT}/recon.png`, fullPage:true});
